...
def online_inference(args: Online):
    ...
    if not choices:
        ...
        model_dump = getattr(chat_completion, "model_dump", None)
        if callable(model_dump):
            pprint(model_dump(), expand_all=True)
        else:
            pprint(chat_completion, expand_all=True)
        print(SEPARATOR)
        raise RuntimeError(
            "Online inference returned no choices. The server accepted the request, "
            "but the response payload did not contain completion choices."
        )

    for output in choices:
        if args.verbose:
            pprint(output, expand_all=True)
        reasoning_content = getattr(output.message, "reasoning_content", None)
        if reasoning_content:
            print(SEPARATOR)
            print("Reasoning:")
            print(textwrap.indent(reasoning_content.strip(), "  "))
        print(SEPARATOR)
        print("Assistant:")
